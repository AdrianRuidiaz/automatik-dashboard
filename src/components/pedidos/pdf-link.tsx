...
interface PdfLinkProps {
  ...
  // (o null) el nombre de archivo cae al numero de pedido.
  nombreCliente?: string | null;
  // Fix 2026-09-01 (decimoprimera vuelta): numero de pedido (id_plataforma),
  // usado como respaldo cuando no hay nombre de cliente disponible -- ver el
  // comentario en src/lib/pdf.ts. Sin el (o null) el nombre de archivo cae
  // directo al ultimo recurso (segmento generico de la URL).
...
export function PdfLink({ url, nombreCliente, numeroPedido, className, children, stopPropagation, as = "button" }: PdfLinkProps) {
  ...
  const activar = async (e: { stopPropagation: () => void }) => {
    ...
    setError(null);
    setCargando(true);
    const resultado = await abrirPdf(url, nombreCliente, numeroPedido);
    setCargando(false);
