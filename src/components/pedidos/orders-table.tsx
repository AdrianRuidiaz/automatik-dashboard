...
function OrderDetail({ pedido }: { pedido: Pedido }) {
  ...
  const handleCancelar = async () => {
        ...
        const confirmado = window.confirm(
                `Cancelar el pedido ${pedido.id_plataforma}? El pedido no se eliminara, solo cambiara su estado a Cancelado.`
              );
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                      <div>
                                ...
                                <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                                            <div><p className="text-xs text-muted-foreground">Cliente</p>p><p className="font-medium truncate">{pedido.cliente_nombre || "-"}</p>p></div>div>
                                            <div><p className="text-xs text-muted-foreground">Fecha</p>p><p>{formatFechaLarga(pedido.fecha_pedido)}</p>p></div>div>
                                            <div><p className="text-xs text-muted-foreground">Total</p>p><p className="font-medium">{formatCLP(pedido.total_pagado)}</p>p></div>div>
                                            <div><p className="text-xs text-muted-foreground">Limite despacho</p>p><p className="text-amber-600 text-xs">{formatFechaLarga(pedido.fecha_limite_despacho)}</p>p></div>div>
                                            <div className="col-span-2"><p className="text-xs text-muted-foreground">Pack ID</p>p><p className="font-mono text-xs break-all">{pedido.id_plataforma}</p>p></div>div>
                                            <div className="col-span-2"><p className="text-xs text-muted-foreground">Ultima actualizacion de estado</p>p><p className="text-xs">{formatFechaLarga(pedido.updated_at)}</p>p></div>div>
                                </div>div>
                        ...
                        <div className="space-y-1">
                          {pedido.items.map((item, i) => (
                                            <div key={i} className="flex items-start justify-between gap-2 text-xs">
                                                                <span className="flex-1"><span className="mr-1 rounded bg-background px-1.5 py-0.5 text-[10px] border border-border">x{item.quantity}</span>span>{item.title}</span>span>
                                                                <span className="text-muted-foreground whitespace-nowrap">{formatCLP(item.unit_price)}</span>span>
                                            </div>div>
                                          ))}
                                        <div className="flex justify-between border-t border-border pt-1 text-xs font-medium mt-2">
                                                          <span>Total</span>span><span>{formatCLP(pedido.total_pagado)}</span>span>
                                        </div>div>
                        </div>div>
                      ) : <p className="text-xs text-muted-foreground">Sin items registrados</p>p>}
                                </div>div>
                      </div>div>
                      ...
                      <div className="flex flex-col gap-2">
                    ...
                    ) : <p className="text-xs text-muted-foreground">Sin etiqueta disponible</p>p>}
                        ...
                        {loading ? <p className="text-xs text-muted-foreground">Cargando...</p>p> : evidencias.length > 0 ? (
                          ...
                          <a key={ev.id} href={getPublicUrl("evidencias", ev.url)} target="_blank" rel="noopener noreferrer"
                                ...
                                ) : <p className="text-xs text-muted-foreground">Sin evidencias</p>p>}
                      </div>div>
                      ...
                      <div>
                        ...
                        {loading ? <p className="text-xs text-muted-foreground">Cargando...</p>p> : documentos.length > 0 ? (
                          ...
                          <a key={doc.id} href={getPublicUrl("documentos", doc.url)} target="_blank" rel="noopener noreferrer"
                                            ...
                                            <div className="min-w-0">
                                                                <p className="font-medium capitalize">{doc.tipo.replace("_", " ")}</p>p>
                                                                <p className="text-muted-foreground truncate">{doc.nombre_archivo}</p>p>
                                            </div>div>
                          </a>a>
                        ))}
                                  </div>div>
                                ) : <p className="text-xs text-muted-foreground">Sin documentos tributarios</p>p>}
                      </div>div>
              </div>div>
        </div>div>
      );
...
export function OrdersTable({ pedidos }: OrdersTableProps) {
    ...
    const columns: ColumnDef<Pedido>[] = useMemo(() => [
      ...
      },
      {
              ...
              header: ({ column }) => (<button className="flex items-center gap-1 font-normal" onClick={() => column.toggleSorting()}>N pedido <ArrowUpDown className="h-3 w-3" /></button>button>),
              cell: ({ row }) => <span className="font-medium text-xs md:text-sm">{row.original.id_plataforma}</span>span>,
      },
      {
              accessorKey: "plataforma", header: "Plat.",
              cell: ({ row }) => <span className={cn("inline-block rounded px-1.5 py-0.5 text-[10px] md:text-[11px]", row.original.plataforma === "ML" ? "bg-ml-light text-ml-dark" : "bg-fa-light text-fa-dark")}>{row.original.plataforma === "ML" ? "ML" : "FA"}</span>span>,
      },
      {
              accessorKey: "fecha_pedido",
              header: ({ column }) => (<button className="hidden md:flex items-center gap-1 font-normal" onClick={() => column.toggleSorting()}>Fecha <ArrowUpDown className="h-3 w-3" /></button>button>),
              cell: ({ row }) => <span className="hidden md:block text-sm">{formatFechaCorta(row.original.fecha_pedido)}</span>span>,
      },
      {
              accessorKey: "cliente_nombre", header: "Cliente",
              cell: ({ row }) => <span className="block max-w-[120px] md:max-w-[180px] truncate text-sm">{row.original.cliente_nombre ?? "-"}</span>span>,
      },
      {
              id: "items_preview", header: "Items",
              cell: ({ row }) => {
                        const t = (row.original.items ?? []).map(i => i.title).join(", ");
                        return <span className="hidden lg:block max-w-[160px] truncate text-xs text-muted-foreground">{t || "-"}</span>span>;
              },
      },
      {
              accessorKey: "total_pagado",
              header: ({ column }) => (<button className="flex items-center gap-1 font-normal" onClick={() => column.toggleSorting()}>Total <ArrowUpDown className="h-3 w-3" /></button>button>),
              cell: ({ row }) => <span className="text-sm whitespace-nowrap">{formatCLP(row.original.total_pagado)}</span>span>,
      },
      {
      ...
      },
      {
              ...
              cell: ({ row }) => row.original.etiqueta_url ? (
                        <button className="inline-flex items-center gap-1 rounded border border-input px-1.5 py-0.5 text-xs text-muted-foreground hover:bg-secondary"
                                  ...
                                  <span className="hidden md:inline">PDF</span>span>
                        </button>button>
    ...
    return (
          <div>
                <div className="mb-4 flex items-center justify-between">
                        <h2 className="text-base font-medium">Pedidos</h2>h2>
                        <span className="text-xs text-muted-foreground">{filtered.length} pedidos</span>span>
                </div>div>
                  ...
                  {activeFilterCount > 0 && (
                      <button onClick={clearFilters} className="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground">
                        ...
                        <button className="ml-auto inline-flex items-center gap-1 rounded-md border border-input px-2.5 py-1.5 text-xs text-muted-foreground hover:text-foreground shrink-0">
                                  <Download className="h-3.5 w-3.5" /> <span className="hidden sm:inline">Exportar</span>span>
                        </button>button>
                </div>div>
            ...
            {showFilters && (
                    <div className="mb-4 rounded-lg border border-border bg-card p-3 space-y-3 animate-in slide-in-from-top-1 duration-200">
                              <div>
                                          <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider mb-1.5">Plataforma</p>p>
                              ...
                              <div>
                                          <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider mb-1.5">Estado</p>p>
                                          <div className="flex gap-1 flex-wrap">
                              ...
                              <div>
                                          ...
                                          <div className="flex flex-wrap items-center gap-2">
                                                        <div className="flex items-center gap-1.5">
                                                                        <label className="text-xs text-muted-foreground">Desde</label>label>
                                                        ...
                                                        <div className="flex items-center gap-1.5">
                                                                        <label className="text-xs text-muted-foreground">Hasta</label>label>
            ...
            {table.getRowModel().rows.length === 0 && (
                    <p className="py-12 text-center text-sm text-muted-foreground">No hay pedidos con esos filtros</p>p>
                )}
                ...
                <div className="mt-3 flex items-center justify-between gap-2 text-xs text-muted-foreground">
                        ...
                        <div className="flex gap-1">
                                  <button onClick={() => table.previousPage()} disabled={!table.getCanPreviousPage()} className="rounded border border-input px-3 py-1 disabled:opacity-40 hover:bg-secondary">Anterior</button>button>
                                  <button onClick={() => table.nextPage()} disabled={!table.getCanNextPage()} className="rounded border border-input px-3 py-1 disabled:opacity-40 hover:bg-secondary">Siguiente</button>button>
                        </div>div>
                </div>div>
          </div>div>
        );
