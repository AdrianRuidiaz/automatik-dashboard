...
import { ArrowUpDown, Search, Download, FileText, ChevronDown, ChevronRight, Package, Info } from "lucide-react";
...
function OrderDetail({ pedido }: { pedido: Pedido }) {
  return (
    <div className="grid grid-cols-1 gap-4 p-4 md:grid-cols-2 bg-card/50">
      <div>
        <h4 className="mb-2 flex items-center gap-1 text-xs font-medium text-muted-foreground">
          <Info className="h-3 w-3" /> Informacion del pedido
        </h4>
      ...
      <div>
          ...
          <div className="space-y-1">
            {pedido.items.map((item, i) => (
              <div key={i} className="flex justify-between text-sm">
                <span><span className="mr-1 rounded bg-secondary px-1.5 py-0.5 text-[10px] text-muted-foreground">x{item.quantity}</span>{item.title}</span>
        ...
        {pedido.etiqueta_url && (
          <a href={pedido.etiqueta_url} target="_blank" rel="noopener noreferrer"
            className="mt-3 inline-flex items-center gap-1 rounded border border-input px-3 py-1.5 text-xs hover:bg-secondary">
            <FileText className="h-3 w-3 text-red-500" /> Descargar etiqueta
          </a>
        )}
      </div>
    </div>
  ...
  const columns: ColumnDef<Pedido>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        header: ({ column }) => (<button className="flex items-center gap-1" onClick={() => column.toggleSorting()}>N\u00b0 pedido <ArrowUpDown className="h-3 w-3" /></button>),
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return <span className={cn("inline-block rounded px-2 py-0.5 text-[11px]", p === "ML" ? "bg-ml-light text-ml-dark" : "bg-fa-light text-fa-dark")}>{p === "ML" ? "ML" : "FA"}</span>;
        },
      },
      {
        accessorKey: "fecha_pedido",
        header: ({ column }) => (<button className="flex items-center gap-1" onClick={() => column.toggleSorting()}>Fecha <ArrowUpDown className="h-3 w-3" /></button>),
      ...
      { accessorKey: "cliente_nombre", header: "Cliente", cell: ({ row }) => row.original.cliente_nombre ?? "\u2014" },
      {
        ...
        cell: ({ row }) => {
          ...
          return <span className="block max-w-[160px] truncate text-xs text-muted-foreground">{text || "\u2014"}</span>;
        },
      },
      {
        accessorKey: "total_pagado",
        header: ({ column }) => (<button className="flex items-center gap-1" onClick={() => column.toggleSorting()}>Total <ArrowUpDown className="h-3 w-3" /></button>),
        cell: ({ row }) => formatCLP(row.original.total_pagado),
      },
      {
        accessorKey: "estado",
        header: "Estado",
        cell: ({ row }) => <span className={cn("inline-block rounded px-2 py-0.5 text-[11px]", ESTADO_COLORS[row.original.estado])}>{ESTADO_LABELS[row.original.estado]}</span>,
      },
      {
        id: "etiqueta",
        header: "Etiqueta",
    ...
    <div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        <div className="flex items-center gap-2 rounded-md border border-input px-3 py-1.5">
          ...
          <input placeholder="Buscar por N\u00b0 pedido, cliente..." value={globalFilter} onChange={(e) => setGlobalFilter(e.target.value)}
        ...
        {(["all", "ML", "Falabella"] as const).map((opt) => (
          <button key={opt} onClick={() => setPlatformFilter(opt)}
            className={cn("rounded-md border px-3 py-1.5 text-xs transition-colors", platformFilter === opt ? "border-primary bg-primary/10 text-primary" : "border-input text-muted-foreground hover:text-foreground")}>
            {opt === "all" ? "Todos" : opt === "ML" ? "ML" : "FA"}
          </button>
        ))}
        <button onClick={() => setOnlyPending((v) => !v)}
          className={cn("rounded-md border px-3 py-1.5 text-xs transition-colors", onlyPending ? "border-primary bg-primary/10 text-primary" : "border-input text-muted-foreground hover:text-foreground")}>
          Pendientes
        </button>
        ...
        <table className="w-full border-collapse text-sm">
          <thead>
            {table.getHeaderGroups().map((hg) => (
              <tr key={hg.id}>
                {hg.headers.map((h) => (
                  <th key={h.id} className="border-b border-border px-2 py-2 text-left text-xs font-normal text-muted-foreground">
          ...
          <tbody>
            {table.getRowModel().rows.map((row) => (
              <>
                <tr key={row.id} className="cursor-pointer border-b border-border hover:bg-secondary/50"
