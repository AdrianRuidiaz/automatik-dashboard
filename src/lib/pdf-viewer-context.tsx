...
export function PdfViewerProvider({ children }: { children: React.ReactNode }) {
  ...
  const abrirPdf = useCallback(async (url: string, nombreCliente?: string | null, numeroPedido?: string | null) => {
    const resultado = await fetchPdfBlob(url, nombreCliente, numeroPedido);
